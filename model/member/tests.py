from django.db.models import Q, Count, Model, Max, Min
...
class MemberTest(TestCase):
    ...
    # 쿼리문 직접 치기
    # query = """
    #     select m.id, m.member_email, m.member_name, count(p.id) post_count, count(r.id) reply_count
    #     from
    #     (
    ...
    # aggregate
    # annotate() 는 QuerySet 객체로 리턴하기 때문에 뒤에 이어서 추가작업이 가능하지만,
    # aggregate()는 전체대상이므로 뒤에 이어서 추가 작업이 불가능하다.
    ...
    # delete
    # try:
    #     count = Member.objects.get(id=1).delete()
    #     print(count)
    # except:
